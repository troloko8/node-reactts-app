...
exports.login = catchAsync(async (req, res, next) => {
    ...
    const user = await User.findOne({ email: email }).select('+password') // to select prop which was set up as unselected in MODEL
...
exports.protect = catchAsync(async (req, res, next) => {
    ...
    if (!token) {
        return next(
            new AppError('You are not logged pls log in to get access'),
            401,
        )
    }
    ...
    if (freshUser.changedPasswordAfter(decodedToken.iat)) {
        // timestamp when was created this toke
        return next(
            new AppError(
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
    ...
    if (!user) {
        return next(
            new AppError('There is no user with this email adress'),
            404,
        )
    }
    // 2) gen random token
